...
function setupEventListeners() {
        ...
        calculateButton.addEventListener('click', () => {
            ...
            if (window.goatcounter && window.goatcounter.count) {
                window.goatcounter.count({
                    path:  'click-calculate-projection',
                    title: 'Clicked Calculate Projection',
                    event: true,
                });
            }
        });
    ...
    if(annualPreFireTabButton) annualPreFireTabButton.addEventListener('click', () => showTab('annualPreFire'));
    if(monthlyPreFireTabButton) monthlyPreFireTabButton.addEventListener('click', () => showTab('monthlyPreFire'));
    if(postFireTabButton) postFireTabButton.addEventListener('click', () => showTab('postFire'));
...
function drawEarlyRetirementPlot(plotDataSets, isComparison = false) {
    ...
    let rightMargin = 80;
    let legendPosition = { x: containerWidth - 70, y: 20 };
    let plotTitlePosition = { x: (containerWidth / 2) - 40, y: -20 }; // Adjusted for legend

    if (isComparison) {
        if (containerWidth > 750) { // Wide screen
            rightMargin = 250;
            legendPosition = { x: containerWidth - 280, y: 0 };
            plotTitlePosition = { x: (containerWidth / 2) - 150, y: 0 - (50 / 2) };
        } else { // Narrow screen
            rightMargin = 50; // Much smaller right margin
            legendPosition = { x: 0, y: -45 }; // Move legend above the plot
            plotTitlePosition = { x: containerWidth / 2 - 60, y: -25 };
        }
    }
    
    const margin = {top: 50, right: rightMargin, bottom: 50, left: 70};
